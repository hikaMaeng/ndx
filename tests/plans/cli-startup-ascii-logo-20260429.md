...
## Goal
Verify that `ndx` startup prints the configured robot startup art before socket/session output.
...
4. Confirm stderr starts with the configured robot startup art before `[socket] connected`.
5. After merge to `main`, rebuild compose with `NDX_GIT_REF=main` and confirm `docker compose logs` shows `git_ref=main`.

## Expected Results
- Unit tests pass.
- `printWelcomeLogo` emits the complete configured startup art.
