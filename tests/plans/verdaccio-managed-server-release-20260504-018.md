# Test Plan: verdaccio-managed-server-release-018
...
Publish `@neurondev/ndx@0.1.18` to Verdaccio after making Windows
managed-server diagnostics best-effort so log write failures cannot block
server startup.
...
- Local package version is `0.1.18`.
- Verdaccio latest before publish is `0.1.17`.
...
7. Confirm Verdaccio latest is `0.1.18`.
8. Install `@neurondev/ndx@0.1.18` into an isolated prefix.
...
- Verdaccio latest moves from `0.1.17` to `0.1.18`.
- Installed binaries print `0.1.18`.
