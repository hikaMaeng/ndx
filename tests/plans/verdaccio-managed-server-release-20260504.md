# Test Plan: verdaccio-managed-server-release
...
Publish `@neurondev/ndx@0.1.16` to Verdaccio after the OS-specific managed
server launcher change, then verify installed binaries report the new version.
...
- Local package version is `0.1.16`.
- Verdaccio latest before publish is `0.1.15`.
...
7. Confirm Verdaccio latest is `0.1.16`.
8. Install `@neurondev/ndx@0.1.16` into an isolated prefix.
...
- Verdaccio latest moves from `0.1.15` to `0.1.16`.
- Installed binaries print `0.1.16`.
