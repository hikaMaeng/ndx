...
the server process. The launcher is OS-specific: Windows uses PowerShell
`Start-Process`, macOS uses `nohup`, Linux uses `setsid` with `nohup` fallback,
and unknown platforms use direct detached Node spawn.
