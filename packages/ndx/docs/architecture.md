...
| `src/agent/loop/artifacts.ts`       | Bounded local artifact reference detection and context rendering.                               |
...
| `src/agent/loop/tool-execution.ts`  | Tool call event emission, argument parsing, ordered task-tool execution, external tool outputs. |
| `src/agent/subagents.ts`            | In-process controller for collaboration and CSV worker agent tools.                             |
...
package scale while keeping NDX-owned external tool execution, MCP discovery,
and runtime event persistence outside the agent loop.
